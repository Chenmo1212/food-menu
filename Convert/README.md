...
这个Python脚本可以将HEIC格式的图片转换为PNG或JPEG格式，并进行压缩和尺寸调整。

## 功能特点

✅ **格式转换**: HEIC → PNG 或 JPEG  
✅ **尺寸调整**: 默认800x600像素（保持宽高比）  
✅ **强力压缩**: 
- PNG: 使用最高压缩级别(9) + 可选pngquant额外压缩
- JPEG: 可调质量设置（默认85）  
✅ **批量处理**: 自动处理文件夹中所有HEIC文件  
...
```python
TARGET_WIDTH = 800          # 目标宽度（像素）
TARGET_HEIGHT = 600         # 目标高度（像素）
JPEG_QUALITY = 85          # JPEG质量 (0-100)
PNG_COMPRESSION = 9        # PNG压缩级别 (0-9)
...
典型的压缩效果：
- **不使用pngquant**: 原始HEIC → PNG，减少约30-50%
- **使用pngquant**: 原始HEIC → PNG，减少约60-80%
- **使用JPEG**: 原始HEIC → JPEG，减少约70-85%

## 示例输出

```
Found 8 HEIC file(s) to convert
Target size: 800x600 (aspect ratio: preserved)
Output format: PNG
